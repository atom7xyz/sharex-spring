...
2. **Download the docker-compose and the application file:**
   ...
   wget https://github.com/atom7xyz/sharex-spring/releases/download/0.0.6/sharex-spring-0.0.6-default.jar
   ```
...
3. **Move the application JAR file to a convenient directory:**
   ...
   mv ./sharex-spring-0.0.6-default.jar ~/tools/sharex-spring/ && \
...
1. **Download the native binary for your platform:**
   ...
   ```shell
   wget https://github.com/atom7xyz/sharex-spring/releases/download/0.0.6/sharex-spring-0.0.6-amd64
   ```

2. **Make it executable:**

   ```shell
   chmod +x sharex-spring-0.0.6-amd64
   ```

3. **Run the application:**

   ```shell
   ./sharex-spring-0.0.6-amd64
   ```
...
4. **Move the compiled JAR file to a convenient directory:**
   ...
   ```shell
   mkdir -p ~/tools/sharex-spring && \
   mv ./build/libs/sharex-spring-0.0.6-default.jar ~/tools/sharex-spring/ && \
   mv ./docker-compose.yml ~/tools/sharex-spring/ && \
   cd ~/tools/sharex-spring/
   ```
...
4. **Run the JAR file:**
   ...
   ```shell
   java -jar sharex-spring-0.0.6-default.jar
   ```
   or
   ```shell
   chmod +x ./start.sh && \
...
5. **Configuration:**
   ...
   java \
     ...
     -Dserver.port=9007 \
     -jar sharex-spring-0.0.6-default.jar
   ```
